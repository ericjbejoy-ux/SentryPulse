"""
Honest accuracy check for app/anomaly.py.

There is no real labeled production data yet (see README_TELEMETRY.md
"Known simplifications"), so this builds a labeled test set using the
exact same distributions telemetry.py uses to simulate NOMINAL vs.
CRITICAL (attacked) traffic, then scores the detector against it at the
FR-4.2 threshold (0.65).
"""
import numpy as np
...
# Same ranges as telemetry.py's tick(): nominal vs attacked
